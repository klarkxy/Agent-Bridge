...
Worker **只能**通过 Agent Bridge 的 MCP 工具调用（`list_agents`、`dispatch_task`、`wait_task` 等）。这一轮的工具列表里如果没有这些工具，停下来告诉用户。**不要**退回去自己跑 `kimi`、`kimi acp`、`grok`、`agy`、`dsh`、`opencode`，或对它们 `python -c`。工具列表缺失是主机 / MCP 的问题，不是准许你直连 Worker。「测一下 Kimi / 协调 Kimi / 试试这个 Worker」仍然是 `dispatch_task`，不是 shell 拉起 CLI。Worker 跑完后你自己跑 `git` / `pytest` 是验收，不能代替派发。
